...
(eval-and-compile
  (require 'cl-lib)
  ;; a very strange problem exists in the compiled cl-seq.elc in
  ;; debian stable's 23.4 emacs.
  (when (and (symbol-function 'position)
             (not (eq (symbol-function 'position)
                      'cl-position)))
    (setf (symbol-function 'cl-position)
